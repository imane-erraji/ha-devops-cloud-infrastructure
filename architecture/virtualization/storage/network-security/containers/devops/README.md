...
1. **Code Checkout** → Git pull from repository.
2. **Build & Test** → Compile code, run unit tests.
3. **Docker Build** → Build application image.
4. **Push to Registry** → Push Docker image to Nexus or GitHub Packages.
5. **Deploy to Kubernetes** → Apply manifests to dev/prod clusters.
6. **Post-Deployment Checks** → Health checks, automated alerts.
